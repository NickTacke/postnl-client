...
import { PostNLError, PostNLRateLimitError, PostNLTimeoutError } from "../../src/core/errors";
...
describe("Transport", () => {
  ...
  it("surfaces internal timeout as PostNLTimeoutError", async () => {
    // honor the abort signal like real fetch so the internal timer can reject
    const fetchMock = mock(
  ...
  it("retries 429 honoring retry-after header then succeeds", async () => {
    const responses = [
    ...
    let i = 0;
    const fetchMock = mock(() => Promise.resolve(responses[i++] as Response));
    // backoffFactor is large; retry-after:0 must drive the (near-zero) wait instead
    const t = new Transport(
      resolveConfig({
        apiKey: "k",
        fetch: fetchMock as unknown as typeof fetch,
        retry: { backoffFactor: 1000 },
      }),
    );
  ...
  it("rejects non-json error body with PostNLError not SyntaxError", async () => {
    const fetchMock = mock(() =>
      Promise.resolve(
